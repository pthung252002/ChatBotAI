...
class Action_CungCap_NguyenNhan_EB(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if current_topic:
            ...
            dispatcher.utter_message(text="Bạn muốn hỏi nguyên nhân của bệnh gì?")
        return []
...
class Action_CungCap_TrieuChung_EB(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if current_topic:
            ...
            dispatcher.utter_message(text="Bạn muốn hỏi triệu chứng của bệnh gì?")
        return []
...
class Action_CungCap_PhongNgua_EB(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if current_topic:
            ...
            dispatcher.utter_message(text="Bạn muốn hỏi cách phòng ngừa bệnh gì?")
        return []
...
class Action_CungCap_TrieuChung_SM(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        current_topic = tracker.get_slot("current_topic")
        if current_topic:
            dispatcher.utter_message(text=f"Triệu chứng của {current_topic}như sau:/n 
			- Giai đoạn đầu: Xuất hiện các chấm nhỏ màu vàng hoặc bạc trên mặt trên của lá, lá có vẻ mất màu, xỉn hơn bình thường./n 
			- Giai đoạn tiến triển: Lá bị vàng úa, khô héo và có thể rụng sớm, xuất hiện mạng tơ mỏng do nhện đỏ giăng trên lá, thân và đọt cây./n 
...
class Action_CungCap_PhongNgua_SM(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        current_topic = tracker.get_slot("current_topic")
        if current_topic:
            dispatcher.utter_message(text=f"Cách phòng ngừa {current_topic}:/n 
			- Tưới nước hợp lý: Nhện đỏ không thích ẩm, nên có thể tăng độ ẩm vườn bằng cách tưới phun sương nhẹ./n 
			- Cắt tỉa lá già, lá bệnh, giúp vườn thông thoáng, giảm nơi trú ẩn của nhện đỏ./n 
